# Game synopsis — Musings from Eternalmoor / The essence of Yew

Reference document for campaign copy and story tone. Last updated from author notes (2026-08-25).

**Spoiler note for site copy:** Do not reveal that forcing the berry causes weird deformations, or that the Seed blooms only when they truly live as the tree / let it have what it longs for. Do not spell that the Key Station unlocks only when guests tell the truth, or that Kehan makes his own Looking Jar when he understands his calling (keeper spoiler). Public copy may hint the station, keys, and cottage jars without explaining how.

---

## Current Lock Keep chapter (active)

### Setup
- Ju and Inky accept Blackbird’s final trial / bargain: exist as a haunted Tree of Life and Death and bring him its Seed of Truth.
- They arrive tethered to a broken branch from the tree.
- The branch lands on Kehan’s narrowboat; Kehan tows them upstream.

### Arrival — no keeper on site
- They reach **The Lock**: closed; **cottage empty**. No lock keeper present.
- Kehan runs around figuring how to get past.
- The branch is thirsty and puts down roots, anchoring the tree to the ground.

### Becoming the Lock Keeper (through Kehan)
- A second boat arrives → a queue. Kehan finds himself helping the guest onward.
- This begins his **unconscious journey** into becoming Keeper of the Lock.
- With a child’s intuition and the tree’s help, he works out **what each guest truly seeks** (Key Station / passage).
- Journey = **becoming** the Lock Keeper via Kehan — not apprenticing under a living keeper on site.

### Key Station (passage — at the lock)
- Reuse the art-deco theatre sidewalk standee (formerly Fermata keys station) beside the lock — now the **Key Station**.
- Guest speaks where they are bound → the screen paints the motif / vision.
- **Unlock only when they tell the truth** → a key releases → past the lock. Soft fail = nothing happens (no red/green arcade lights).
- Public site: folded into the single **Life by the Lock** bump; hint the station and keys; do **not** spell the truth rule.

### Looking Jars / Keepers past (cottage mystery — not the gate; off public site for now)
- Glass jars inside the empty cottage: a **museum of past lock keepers’ visions for the place** (dioramas / miniatures under glass — museum-case or lantern housing, not crystal-ball silhouette).
- Outdoor ruins / rings of earlier Lock Keep variants.
- **Public site:** Keepers past bump removed for now (too much information). Lore stays here only.
- **Design spoiler / end beat:** when Kehan understands his calling, he makes **his own** Looking Jar. Do **not** put this on the public site (keeper spoiler).

### End beat (thematic mirror for Ju)
- Kehan asks where *he* is headed — and realizes he has already arrived: he wants a life **in service of others**, perfecting the trait of lock keeper.
- Hint to Ju: sometimes you already have what you’re seeking (her iterations through lives vs becoming Blackbird who watches the Theatre).
- He creates his Looking Jar as the quiet seal of that calling (synopsis / game only).

### Environment / lore (not all on the public site)
- Lock-side: Key Station (active passage instrument).
- Cottage: Looking Jars museum of keepers past (unnamed on public site — no Nevlari character).
- Outside: ruins / rings of earlier Lock Keep variants.

### Mission / stakes (public)
- No separate mission list (it repeated the bargain / Seed of Truth).
- Stakes line (from earlier site copy): Your haunting dictates whether the Lock Keep spirals into cosy whimsy or dark, gritty tragedy.
- Shorter historical variants: "Your haunting steers the Lock Keep toward whimsy or tragedy." / "…steers the cottage toward whimsy or tragedy."
- Bargain pitch stays short. Keepers past / undoing mission lines: off public page for now.

### Quest bumps (public)
- **A lock with a will of its own**  
  Young Kehan arrives at a mysterious abandoned lock that won't open, though every mechanism looks sound. A Key Station stands by the lock, revealing images that shift with the person looking. Help him figure out what makes its keys fall for the gates — and get the waiting guests through.
- **Survive the ricochet of your own haunting.**  
  The Lock Keep asks much of whoever tends it and whoever seeks passage — and plays both for fools when they fall short. Mind your counsel: what you tend (or overlook) can ricochet — along this canal, every assumption has a habit of landing beautifully wrong.

### Seed of Truth (design spoiler)
- Forcing the berry from the tree → odd deformations; no true Seed.
- Living as the tree / letting it pursue what it longs for → the Seed appears of its own accord.
- Ju’s essence: she thinks she wants to FIND all answers; she actually burns to SEEK — the hunt itself.

---

## Older outline (partly superseded)

The sections below retain earlier Nevlari-on-site / memorial Looking Jar beats. Prefer **Current Lock Keep chapter** when they conflict.

## Eternalmoor och val av minnen

Spelet inleds på Eternalmoor där Hero har avslutat sitt liv som H. Spelaren väljer om H:s minnen ska tas med in i nästa akt i livets teater.

## Teaterscenen och blixtnedslaget

Under en storm på scenen ser Hero en dörrmus söka skydd i ett träd. När blixten slår ner i trädet och det fattar eld, reinkarnerar Hero in i trädet. Hero leder musen ut över en gren som sträcker sig till bristningsgränsen ut över kanalen, tills den bryts av sin tyngd och landar på en kanalbåt.

## Uppvaknandet och H:s död

Smällen väcker barnet Kehan som upptäcker elden och springer för att väcka sin vårdare H, vilken har dött av ålder i sömnen.

## Färden och begravningen (äldre — Nevlari på plats)

Den rådiga Kehan kopplar loss hästen, drar pråmen uppströms med grenen och musen ombord till slussvaktens stuga där Nevlari tar emot.

Nevlari har tillgång till ett arkiv av sista önskningar från alla som arbetar vid den olycksdrabbade kanalen (klämskador, fall, osv.). Önskningarna används som inspiration för tillverkning av en Looking jar, ett slags minneskapsel för dem som sällan har råd med riktiga begravningar. Looking jar är en glasburk med miniatyrvärld inuti och en speldosa med en liten melodi och en låda med en liten pryl från personen.

Kehan får skapa H:s looking jar och vid H:s enkla begravning placeras den ut tillsammans med den vackra grenen och musen i minneslunden intill Slussen.

## Trädets tillväxt och kärlek

Grenen slår rot. Kehan och Nevlari och alla resande förundras över dess magiskt snabba och dramatiska tillväxt. Trädet blir snabbt en plats för människor att pausera som vill dryfta stort och smått i dess skugga. Nevlari som själv är gammal ser fram emot vila: att få välja path of Fermata (avsluta cykeln av awareness). Kehan argumenterar emot — han har öga för att njuta av livet trots den bistra vardagen och delar Heros sprudlande nyfikenhet.

Efter en period av miljöutforskning, sysslor intill kanalen (öppna för passerande båtar / bjuda på kaffe / fånga upp saker som faller överbord / spjälka brutna fingrar / vårda arbetande klämskador etc.) och samtal om livets mening med människorna som rör sig omkring trädet, blommar en kärlekshistoria ut mellan dörrmusen och det fullvuxna trädet som slår ut i blom.

## Tragedin med draghästen

Kehans bäste vän och draghäst äter av trädets röda bär (med dess giftiga svarta kärna) på natten och hittas svårt sjuk på morgonen, vilket kastar Hero in i skuld och Kehan in i djup sorg och misstro.

## Valley of despair

När Inky (Heros hjärta) förlorar hoppet ställs Hero inför ett val: att hjälpa hjärtat återfinna sin livsglädje, eller utnyttja Inky-apatin för att smita in backstage till Mr. Blackbird för att få svar på livets frågor — utan att kunna ta med Inky.

Kostnaden står klar: att arbeta som en allknowing blackbird backstage är att inte längre ha koppling till det som sker på scenen; ingen nyfikenhet kan existera där allt är känt. Sanningen prövas i om kärleken och hoppet vinner över jakten på ett facit.

Hero inser, kan hända, att hon inte brinner för att faktiskt HITTA alla svar, utan brinner för att SÖKA dem — vilket är två helt olika saker.

När en veterinär/läkare kommer förbi för att hjälpa hästen bli bra igen, känner veterinären igen trädet som yew tree. Hen säger att bären är eftertraktade som ingrediens i cancermedicin och ber att få börja transportera bär uppströms.

Hästen pensioneras. Kehan "adopteras" av Nevlari och tar vid som slussväktare istället. Varje vår hjälps de åt att plocka yew berries, så att de kommer till bra användning och inte riskerar någons liv.
